...
import { Coins, Sparkles, Star } from '../../components/ui/Icon'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      <ToolHeader
        eyebrow="DIVINATION"
        title="算卦"
        desc="六爻摇卦 · 梅花易数 · 诚心所至，卦象自明。"
      />
      {/* Hub — method selection */}
      <div className="hub-grid">
        <div className="hub-card" onClick={() => setView('liuyao')}>
          <div className="hub-icon" style={{ background: 'rgba(184,150,15,0.12)', color: 'var(--hu-po-jin-dark)' }}>
            <Coins size={24} />
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, justifyContent: 'center', marginTop: 14 }}>
            <span className="ds-chip ds-chip-ink">摇卦</span>
            <span className="ds-chip ds-chip-ink">数字</span>
            <span className="ds-chip ds-chip-ink">随机</span>
          </div>
        </div>

        <div className="hub-card" onClick={() => setView('meihua')}>
          <div className="hub-icon" style={{ background: 'rgba(156,61,84,0.1)', color: 'var(--zhu-sha)' }}>
            <Sparkles size={24} />
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, justifyContent: 'center', marginTop: 14 }}>
            <span className="ds-chip ds-chip-ink">数字</span>
            <span className="ds-chip ds-chip-ink">时间</span>
            <span className="ds-chip ds-chip-ink">文字</span>
          </div>
        </div>
      </div>
      ...
      <div className="section">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <h3 className="font-serif text-lg font-bold" style={{ color: 'var(--dai-qing)', marginBottom: 0 }}>算卦记录</h3>
        ...
        {history.length === 0 && (
          <p style={{ fontSize: 14, color: 'rgba(0,77,77,0.45)', padding: '8px 0' }}>
              ...
              <div key={r.id} className="history-row" onClick={() => handleViewRecord(r)}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginTop: 4, alignItems: 'center' }}>
                    <span className="ds-chip ds-chip-zhong">{r.type === 'liuyao' ? '六爻' : '梅花易数'}</span>
                    <span style={{ fontSize: 11, color: 'rgba(0,77,77,0.45)' }}>{new Date(r.createdAt).toLocaleDateString('zh-CN')}</span>
                  </div>
                </div>
      ...
      {/* Notice */}
      <div style={{ fontSize: 13, color: 'rgba(0,77,77,0.45)', lineHeight: 1.8 }}>
        <p style={{ fontWeight: 500, color: 'rgba(0,77,77,0.6)', marginBottom: 6 }}>起卦须知</p>
